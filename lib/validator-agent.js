...
class ValidatorAgent {
  constructor(options = {}) {
    ...
    this.allowMock = options.allowMock === true || this.apiKey === 'mock-key';
  ...
  async validateFinding(finding) {
    ...
    try {
      ...
      const response = await this._callClaude(prompt, finding);
...
DEFENSE_CHECKS: [defensive code found, or "None"]`;
  ...
  /**
   * Call Claude API with real Anthropic SDK or mock response
   */
  async _callClaude(prompt, finding = {}) {
    if (this.allowMock) {
      const { FableModelSimulator } = require('./fable-model-simulator');
      return FableModelSimulator.simulateValidation(finding);
    }
